...
__all__ = ['evaluate_accuracy', 'squared_loss', 'grad_clipping', 'sgd', 'train_and_predict_rnn', 'train_ch3', 'train_ch5', 'to_onehot' , 'predict_rnn', 'train_and_predict_rnn_gluon', 'predict_rnn_gluon', 'grad_clipping_gluon']
...
def grad_clipping_gluon(model, theta, device):
...
def predict_rnn_gluon(prefix, num_chars, batch_size, num_hiddens, model, vocab, device):
...
def train_and_predict_rnn_gluon(model, num_hiddens, init_gru_state, corpus_indices, vocab,
    ...
    loss =  nn.CrossEntropyLoss()
    # loss.float()
    # model.initialize(device=device, force_reinit=True, init=init.Normal(0.01))
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for X, Y in data_iter:
            ...
            with torch.no_grad():
                # Clip the gradient
                grad_clipping_gluon(model, clipping_theta, device)
        ...
        if epoch % (num_epochs // 2) == 0:
            for prefix in prefixes:
                print(' -', predict_rnn_gluon(prefix, 50, batch_size, num_hiddens, model, vocab, device))
