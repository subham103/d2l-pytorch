...
__all__ = ['sgd', 'train_and_predict_rnn', 'to_onehot','grad_clipping','predict_rnn']
...
def train_and_predict_rnn(rnn, get_params, init_rnn_state, num_hiddens,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for X, Y in data_iter:
            ...
            # print(outputs.requires_grad,'hahahhahaha', state)
            ...
            sgd(params, lr, 1)
            # print(params, 'hahhhhhhh')
            # for param in params:
            #     # print('1')
            #     param.grad.data.zero_()
            # print('ssdfgh',params)
            # Since the error is the mean, no need to average gradients here
            l_sum += l * torch.Tensor(list(y.size()))
            n += torch.Tensor(list(y.size()))
...
def predict_rnn(prefix, num_chars, rnn, params, init_rnn_state,
    ...
    for t in range(num_chars + len(prefix) - 1):
        ...
        if t < len(prefix) - 1:
            ...
            output.append(int(Y[0].argmax(1)))
